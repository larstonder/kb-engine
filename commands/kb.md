...
argument-hint: "init <dir> [--preset general|monorepo] [--mode standalone|submodule|inrepo] [--auto-commit|--no-auto-commit] [--force] [--branch <b>]  |  sync  |  doctor"
allowed-tools: Bash
---
...
`/kb init <dir> [--preset general|monorepo] [--mode standalone|submodule|inrepo] [--auto-commit|--no-auto-commit] [--force] [--branch <b>]`, `/kb sync`, and `/kb doctor`.
...
- **`doctor`** - relay its `ok`/`warn`/`fail` report; it checks dependencies, `.kbconfig`, `kb.json`, the vendored validators, and git state.
- **Failed** - show the error and the fix: valid `--preset` is `general` or `monorepo`; valid `--mode` is `standalone`, `submodule`, or `inrepo`. `inrepo` tracks the KB inside the main repo and never pushes; `AUTO_COMMIT` defaults to `false` for `inrepo` (override with `--auto-commit`).
